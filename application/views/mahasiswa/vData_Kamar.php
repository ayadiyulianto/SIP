                              	...
                              	<div class="row">
                              		<div class="col-lg-4">
                              			<?php foreach(${"lantai".$row->id_lantai}->result() as ${"floor".$row->id_lantai}){
								        	...
								        	if($jumlah_penghuni < $maks_penghuni_per_kamar){ ?>
			                                <span style="margin: 5px" <?php if($jumlah_penghuni == 0){ ?> class="btn btn-info"<?php }else{ ?> class="btn btn-warning"<?php }?>>
		                                ...
		                                <table class="myTable table table-bordered table-striped table-condensed table-hover">
		                                    ...
		                                    <tbody>
								              <?php foreach(${"lantai".$row->id_lantai}->result() as ${"lantai".$row->id_lantai}){
								              	$jumlah_penghuni = ${"lantai".$row->id_lantai}->jumlah_penghuni;
								              	if($jumlah_penghuni < $maks_penghuni_per_kamar){ ?>
								                <tr>
								                  <td><h4><?= ${"lantai".$row->id_lantai}->id_kamar; ?></h4></td>
								                  <td>
								                  	<a class="btn btn-sm btn-primary" href="<?= base_url('mahasiswaprocess/pilihkamar/'.${"lantai".$row->id_lantai}->id_kamar)?>"><i class="fa fa-check"> Pilih</i></a>
								                  </td>
								                  <td>
